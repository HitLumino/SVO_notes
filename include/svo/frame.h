...
public:
  ...
  ImgPyr                        img_pyr_;               //!< Image Pyramid. vector<cv::Mat>
  Features                      fts_;                   //!< vikit.
  ...
  g2oFrameSE3*                  v_kf_;                  //!< Temporary pointer to the g2o node object of the keyframe. typedef g2o::VertexSE3Expmap g2oFrameSE3;
  ...
  /// Initialize new frame and create image pyramid. 初始化新的关键帧并且 创建图像金字塔
  void initFrame(const cv::Mat& img);

  /// Select this frame as keyframe. 将该帧设为关键帧
  void setKeyframe();
  ...
  /// Check if a point in (w)orld coordinate frame is visible in the image. 世界坐标系中的点是否在该图像中可见
  bool isVisible(const Vector3d& xyz_w) const;

  /// Full resolution image stored in the frame. 返回金字塔的第一层(最底层)
  inline const cv::Mat& img() const { return img_pyr_[0]; }

  /// Was this frame selected as keyframe? 是否选为关键帧
  inline bool isKeyframe() const { return is_keyframe_; }

  /// Transforms point coordinates in world-frame (w) to camera pixel coordinates (c).  世界坐标系(3d) --> 像素坐标系(2d)
