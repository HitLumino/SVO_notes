...
public:
  ...
  KltHomographyInit() {};
  ~KltHomographyInit() {};
  InitResult addFirstFrame(FramePtr frame_ref);//光流法处理第一帧
  InitResult addSecondFrame(FramePtr frame_ref);//光流法处理第二帧
  void reset();

protected:
  vector<cv::Point2f> px_ref_;      //!< keypoints to be tracked in reference frame.参考特征点
  vector<cv::Point2f> px_cur_;      //!< tracked keypoints in current frame.当前帧特征点
  vector<Vector3d> f_ref_;          //!< bearing vectors corresponding to the keypoints in the reference image. 特征点的方位向量
  vector<Vector3d> f_cur_;          //!< bearing vectors corresponding to the keypoints in the current image.
  vector<double> disparities_;      //!< disparity between first and second frame.前两帧的视差
