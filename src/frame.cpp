...
void Frame::initFrame(const cv::Mat& img)
{
  ...
  /// Set keypoints to NULL 关键点设为null
  std::for_each(key_pts_.begin(), key_pts_.end(), [&](Feature* ftr){ ftr=NULL; });

  /// Build Image Pyramid
  frame_utils::createImgPyramid(img, max(Config::nPyrLevels(), Config::kltMaxLevel()+1), img_pyr_);// 3  4+1
}
...
void Frame::setKeyPoints()
{
  for(size_t i = 0; i < 5; ++i)
    if(key_pts_[i] != NULL)      ///vector<Feature*>          key_pts_;
      if(key_pts_[i]->point == NULL)//如果特征点的坐标是null,则整个feature设为null
        key_pts_[i] = NULL;
...
bool getSceneDepth(const Frame& frame, double& depth_mean, double& depth_min)
{
  ...
  depth_min = std::numeric_limits<double>::max();//最大值
