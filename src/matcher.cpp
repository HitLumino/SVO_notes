    ...
    Matrix2d& A_cur_ref)
{
  ...
  const Vector3d xyz_ref(f_ref*depth_ref); // 得到pixel f 在ref frame 中的3d 坐标
  Vector3d xyz_du_ref(cam_ref.cam2world(px_ref + Vector2d(halfpatch_size,0)*(1<<level_ref))); // 将像素f 在横轴方向扩展几个像素px_ref.u + halfpatch_size，并得到该点的3d坐标
  Vector3d xyz_dv_ref(cam_ref.cam2world(px_ref + Vector2d(0,halfpatch_size)*(1<<level_ref))); // 将像素f 在 纵轴方向扩展几个像素px_ref.v + halfpatch_size，并得到该点的3d坐标
  xyz_du_ref *= xyz_ref[2]/xyz_du_ref[2]; // 将这两个3d坐标归一化到pixel f 同一深度
  xyz_dv_ref *= xyz_ref[2]/xyz_dv_ref[2];
  const Vector2d px_cur(cam_cur.world2cam(T_cur_ref*(xyz_ref)));  // 将这个点转化到cur frame中，并投影到像素坐标
  const Vector2d px_du(cam_cur.world2cam(T_cur_ref*(xyz_du_ref)));
  const Vector2d px_dv(cam_cur.world2cam(T_cur_ref*(xyz_dv_ref)));
  A_cur_ref.col(0) = (px_du - px_cur)/halfpatch_size; // cur frame和ref frame 相比，横坐标拉伸比例。
    ...
    double& depth)
{
  ...
  if(ref_ftr.type == Feature::EDGELET && options_.epi_search_edgelet_filtering)
  {
	...
	// 主特征方向计算方式:
	// Run a Histogram of Oriented Gradients over patches of the image - the peak in each of those histograms will give you the dominant direction of that patch 
    const double cosangle = fabs(grad_cur.dot(epi_dir_.normalized())); // 特征主梯度方向和极线方向的夹角
    if(cosangle < options_.epi_search_edgelet_max_angle) { // 夹角不能太大
  ...
  epi_length_ = (px_A-px_B).norm() / (1<<search_level_); // 极线搜索长度
  ...
  if(epi_length_ < 2.0) // 极线长度小于两个像素
  {
    px_cur_ = (px_A+px_B)/2.0;
    Vector2d px_scaled(px_cur_/(1<<search_level_));
    bool res;
    if(options_.align_1d)
		//  (px_A-px_B).cast<float>().normalized() : 1d search direction, along the epi
