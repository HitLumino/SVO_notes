    ...
    size_t& num_obs)
{
  ...
  for(auto it=frame->fts_.begin(); it!=frame->fts_.end(); ++it)
  {
    if((*it)->point == NULL)
      continue;
    // it->f : 特征的位置 f = [(u-ox)/fx , (v-oy)/fy , 1]
	// (*it)->point->pos_ : 三维点位置
    Vector2d e = vk::project2d((*it)->f)
               - vk::project2d(frame->T_f_w_ * (*it)->point->pos_);
    e *= 1.0 / (1<<(*it)->level);
    errors.push_back(e.norm());//二范数
  }
  if(errors.empty())
    return;
  vk::robust_cost::MADScaleEstimator scale_estimator;
  estimated_scale = scale_estimator.compute(errors);//NORMALIZER * vk::getMedian(errors)  NORMALIZER=1.48
