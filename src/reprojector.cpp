    ...
    std::vector< std::pair<FramePtr,std::size_t> >& overlap_kfs)
{
  ...
  map_.getCloseKeyframes(frame, close_kfs);// 通过图像中是否有共同观测到的keypoint 获取CloseKeyframes
      ...
      it_frame!=ite_frame && n<options_.max_n_kfs; ++it_frame, ++n)
  {
        ...
        it_ftr!=ite_ftr; ++it_ftr)
    {
      ...
      if(reprojectPoint(frame, (*it_ftr)->point))// 记录三维点投影后的单元格
...
bool Reprojector::reprojectCell(Cell& cell, FramePtr frame)
{
  ...
  Cell::iterator it=cell.begin(); // cell 中保存了多个投影点
  while(it!=cell.end())
  {
    ++n_trials_;
    ...
    if(!found_match) // 如果这个点附近没有好的优化位置，则删掉这个点继续寻找
    {
