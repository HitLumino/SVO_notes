...
void DepthFilter::updateSeeds(FramePtr frame)
{
  ...
  double px_noise = 1.0;
  // px_error_angle:  <<REMODE: Probabilistic, Monocular Dense Reconstruction in Real Time>>,  equation[16]
  double px_error_angle = atan(px_noise/(2.0*focal_length))*2.0; // law of chord (sehnensatz)  

  while( it!=seeds_.end()) // 更新当前KF中的种子点
  {
...
//<<REMODE: Probabilistic, Monocular Dense Reconstruction in Real Time>> equation[16] ~ [19]
